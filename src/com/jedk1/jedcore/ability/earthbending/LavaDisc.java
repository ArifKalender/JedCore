...
public class LavaDisc extends LavaAbility implements AddonAbility {
    ...
    private void doDamage(Entity entity) {
        ...
        entity.getWorld().spawnParticle(Particle.LAVA, entity.getLocation(), 3, Math.random(), Math.random(), Math.random(), 0.1f);
    }
    ...
    private class DiscRenderer {
        ...
        void render(Location location, boolean largeLava) {
            ...
            for (Location l : JCMethods.getCirclePoints(location, 10, 0.5, angle)) {
                l.getWorld().spawnParticle(Particle.FLAME, l, 1, 0, 0, 0, 0.01);
                l.getWorld().spawnParticle(Particle.SMOKE_NORMAL, l, 1, 0, 0, 0, 0.05);
                if (largeLava && damageBlocks)
                    damageBlocks(l);
            }
        }
        ...
        private void damageBlocks(Location l) {
            ...
            if (!RegionProtection.isRegionProtected(player, l, LavaDisc.this)) {
                if ((isEarthbendable(player, block) || isMetal(block) || meltable.contains(block.getType().name()))) {
                    ...
                    l.getWorld().spawnParticle(Particle.LAVA, l, particles, Math.random(), Math.random(), Math.random(), 0.2);
